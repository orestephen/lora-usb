...
__attribute__((weak)) void radio_on_dio_irq(void *context) { irq_fired = true; }
__attribute__((weak)) void on_tx_done(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_rx_done(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_preamble_detected(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_syncword_valid(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_header_valid(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_header_error(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_crc_error(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_rx_timeout(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_rx_error(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_cad_done_undetected(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_cad_done_detected(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}
__attribute__((weak)) void on_fhss_hop_done(void) {
  HAL_DBG_TRACE_INFO("%s\n", __func__);
}

sx126x_hal_context_t *apps_common_sx126x_get_context() {
  context.spi_handle = &hspi1;
  context.busy_pin = LORA_BUSY_Pin;
  context.busy_port = LORA_BUSY_GPIO_Port;
  context.reset_pin = LORA_nRESET_Pin;
  context.reset_port = LORA_nRESET_GPIO_Port;
  context.cs_pin = LORA_nCS_Pin;
  context.cs_port = LORA_nCS_GPIO_Port;
  context.ant_pin = LORA_ANT_SW_Pin;
  context.ant_port = LORA_ANT_SW_GPIO_Port;

  return &context;
}

void apps_common_shield_init(void) {
  if (context.ant_port != NULL) {
    HAL_GPIO_WritePin(context.ant_port, context.ant_pin, GPIO_PIN_SET);
  }
}
...
void apps_common_sx126x_print_version_info(void) {
  HAL_DBG_TRACE_INFO("%s\n", sx126x_driver_version_get_version_string());
}
...
void apps_common_sx126x_init(const sx126x_hal_context_t *context) {
  ...
  const bool dio2_is_set_as_rf_switch =
      smtc_shield_sx126x_is_dio2_set_as_rf_switch(&shield);
  ASSERT_SX126X_RC(
      sx126x_set_dio2_as_rf_sw_ctrl(context, dio2_is_set_as_rf_switch));

  const smtc_shield_sx126x_xosc_cfg_t *xosc_cfg =
      smtc_shield_sx126x_get_xosc_cfg(&shield);
  if (xosc_cfg->tcxo_is_radio_controlled == true) {
    ASSERT_SX126X_RC(
        sx126x_set_dio3_as_tcxo_ctrl(context, xosc_cfg->supply_voltage,
                                     xosc_cfg->startup_time_in_tick);)
...
void apps_common_sx126x_radio_init(const void *context) {
  const smtc_shield_sx126x_pa_pwr_cfg_t *pa_pwr_cfg =
      smtc_shield_sx126x_get_pa_pwr_cfg(&shield, RF_FREQ_IN_HZ,
                                        TX_OUTPUT_POWER_DBM);
...
uint32_t get_time_on_air_in_ms(void) {
  ...
  case SX126X_PKT_TYPE_LORA: {
    return sx126x_get_lora_time_on_air_in_ms(&lora_pkt_params,
                                             &lora_mod_params);
  }
  case SX126X_PKT_TYPE_GFSK: {
    return sx126x_get_gfsk_time_on_air_in_ms(&gfsk_pkt_params,
                                             &gfsk_mod_params);
  }
