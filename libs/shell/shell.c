...
static void shellInsert(shellContext_t* pContext, uint8_t data) {
    if (pContext->bufferOffset < (pContext->bufferLength - 1)) {
        ...
        // pContext->iface.print("%c[%d;%dH", SHELL_VT100_ASCII_ESC, 1, (uint8_t)pContext->bufferOffset);
...
void shellParser(shellContext_t* pContext, uint8_t* pCommand) {
    if (pContext != NULL) {
        ...
        for (pProgram = pContext->pTable; pProgram != NULL; pProgram = pProgram->pNext) {
...
int8_t shellProcess(shellContext_t* pContext, char ch) {
    ...
    if (pContext != NULL) {
        ...
        // if (ch == SHELL_VT100_ASCII_BSPACE) {
        //     // backspace found
        //     if (pContext->bufferOffset > 0) {
        //         // buffer is not empty, remove from shell and buffer
        //         pContext->iface.putChar(SHELL_VT100_ASCII_BSPACE);
        //         pContext->iface.putChar(SPACE);
        //         pContext->iface.putChar(SHELL_VT100_ASCII_BSPACE);
        //         pContext->iface.flush();
        ...
        //     if (pContext->bufferOffset < (pContext->bufferLength - 1)) {
        //         // buffer is not full, echo back and store in buffer

        //         pContext->iface.putChar(ch);
        //         pContext->iface.flush();
        //         pContext->pBuffer[pContext->bufferOffset++] = ch;
        //     } else {
        //         // buffer full, do nothing
        //     }
        // } else if (ch == LF) {
